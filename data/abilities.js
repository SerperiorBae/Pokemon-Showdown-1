...
exports.BattleAbilities = {
	...
	"draconiate": {
		...
		onModifyMove: function (move, pokemon) {
			if (move.type === 'Normal' && move.id !== 'naturalgift') {
				move.type = 'Dragon';
				if (move.category !== 'Status') pokemon.addVolatile('draconiate');
			}
		},
		effect: {
			duration: 1,
	...
	"psysorb": {
		desc: "This Pokemon is immune to Psychic-type moves and restores 1/4 of its maximum HP, rounded down, when hit by a Psychic-type move.",
		shortDesc: "This Pokemon heals 1/4 of its max HP when hit by Psychic moves; Psychic immunity.",
		onTryHit: function (target, source, move) {
